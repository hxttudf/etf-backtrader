...
def _make_data_feed(prices, name, open_prices=None):
    """创建 backtrader 数据源。

    - close: ffill（避免跨市场节假日指标断裂）
    - open: 使用真实开盘价（如有），否则用前一日 close 近似
    - openinterest: 0 = 当日原始数据为 NaN，1 = 有效
      策略据此跳过 NaN 日的信号生成，与手写引擎行为一致。
    """
    raw = prices[name]
    ...
    if open_prices is not None and name in open_prices.columns:
        ...
        open_aligned = open_aligned.fillna(close_filled)
...
def _execute_trade(strat, dt, new_holding):
    """Execute a position change at current bar. Backtrader will fill orders at next bar's open = T+1."""
...
def _setup_cerebro(prices, mode, ma_days, roc_days, min_hold=0, strategy='momentum', open_prices=None):
    """创建并配置 Cerebro 实例"""
    cerebro = bt.Cerebro()
...
def run_backtest_bt(prices, mode, start_date, end_date, ma_days=60, roc_days=25, min_hold=0, strategy='momentum', open_prices=None):
    """回测接口。strategy: 'momentum'|'rsi'|'bb'|'macd'|...

    返回 (nav, bench_nav, ret, bench_ret, trades, trade_details, daily_signals)
    """
    etf_names = list(prices.columns)
    cerebro = _setup_cerebro(prices, mode, ma_days, roc_days, min_hold, strategy, open_prices=open_prices)
    results = cerebro.run()
    strat = results[0]
    return _convert_output(strat, prices, start_date, end_date, etf_names)


def position_dist_bt(prices, start_date, end_date, mode, ma_days=60, roc_days=25, min_hold=0, strategy='momentum', open_prices=None):
